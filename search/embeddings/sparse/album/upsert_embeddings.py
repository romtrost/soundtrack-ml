...
albums_dedup = data.drop_duplicates(subset=['album_name'], keep='first').reset_index(drop=True)
print(f"Original number of tracks: {len(data)}")
print(f"Deduplicated number of albums: {len(albums_dedup)}")
...
for i in range(0, len(albums_dedup), BATCH_SIZE):
    batch_points = []
    for j in range(i, min(i + BATCH_SIZE, len(albums_dedup))):
        ...
        batch_points.append(
            PointStruct(
                ...
                payload={
                    "album_name": albums_dedup.iloc[j]['album_name'],
                }
            )
        )
    ...
    if (i // BATCH_SIZE + 1) % 10 == 0:
        print(f"Uploaded {i + BATCH_SIZE} / {len(albums_dedup)} embeddings...")

print(f"Successfully uploaded {len(albums_dedup)} embeddings to Qdrant!")