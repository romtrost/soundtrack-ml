...
corpus = []
for idx, row in albums_dedup.iterrows():
    if idx % 1000 == 0:
        print(f"Processing album {idx}/{len(albums_dedup)}")
    album_name = str(row['album_name']) if pd.notna(row['album_name']) else ''
    corpus.append(tokenize(album_name, config['bm25']['ngram_range']))
print("Corpus: ", corpus[0:30])

# Create sparse vector embeddings using BM25
bm25_album = BM25Okapi(corpus)
