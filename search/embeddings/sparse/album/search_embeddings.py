...
# Search function
def search_albums(query, top_k=10):
    ...
    for hit in search_results:
        results.append({
            "album_name": hit.payload["album_name"],
            "score": hit.score
        })
...
query = "Views"
print(f"\nSearching for: '{query}'")
results = search_albums(query, top_k=10)
print(f"\nSearch results for: '{query}'")
for i, result in enumerate(results, 1):
    print(f"{i}. {result['album_name']} (score: {result['score']:.4f})")
