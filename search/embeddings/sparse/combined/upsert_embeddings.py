...
for i in range(0, len(combined_dedup), BATCH_SIZE):
    batch_points = []
    for j in range(i, min(i + BATCH_SIZE, len(combined_dedup))):
        ...
        batch_points.append(
            PointStruct(
                ...
                payload={
                    "track_name": combined_dedup.iloc[j]['track_name'],
                    "artist_name": combined_dedup.iloc[j]['artist_name'],
                    "album_name": combined_dedup.iloc[j]['album_name']
                }
            )
        )
    ...
    if (i // BATCH_SIZE + 1) % 10 == 0:
        print(f"Uploaded {i + BATCH_SIZE} / {len(combined_dedup)} embeddings...")

print(f"Successfully uploaded {len(combined_dedup)} embeddings to Qdrant!")