...
corpus = []
for idx, row in artists_dedup.iterrows():
    if idx % 1000 == 0:
        print(f"Processing artist {idx}/{len(artists_dedup)}")
    artist_name = str(row['artist_name']) if pd.notna(row['artist_name']) else ''
    corpus.append(tokenize(artist_name, config['bm25']['ngram_range']))
print("Corpus: ", corpus[0:30])

# Create sparse vector embeddings using BM25
bm25_artist = BM25Okapi(corpus)
